function videoSummaries(npPath,subdirectory,ylen,xlen,depth,emPath)
...
for blocki=1:NBlocks
    ...
    generateVideo(segmentation, emFolder, folderPath);
    
end

function generateVideo(dataset1,dataset2,path)
...
datasetF = fullfile(path,strcat('VidSumm_',dataset1,'.mj2'));
if exist(datasetF,'file'), delete(datasetF); end
%%% slow frame by frame rendering
figure('color','w','position',[492    46   847   759]);
v = VideoWriter(datasetF, 'Motion JPEG 2000'  ); % any quality and format can be defined here
%v.Quality=100; 
