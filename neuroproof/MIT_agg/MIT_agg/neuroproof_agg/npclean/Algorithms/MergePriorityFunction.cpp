...
void ProbPriority::clear_dirty()
{
    ...
    // NOTE(TFK): Eliminated parallelism here to get determinism.
    //cilk_for (vector<unsigned long int>::iterator it = delete_vec.begin(); it != delete_vec.end(); ++it) {

    //for (vector<unsigned long int>::iterator it = delete_vec.begin(); it != delete_vec.end(); ++it) {
      std::pair<double, std::pair<Node_t, Node_t> >* temporary_pointers =
          (std::pair<double, std::pair<Node_t, Node_t> >*) malloc(sizeof(std::pair<double, std::pair<Node_t, Node_t> >) * delete_vec.size());
 
      cilk_for (int i = 0; i < delete_vec.size(); i++) {
        Node_t node1, node2;
        //pairing_func_inverse_num1(*it, &node1, &node2);
        pairing_func_inverse_num1(delete_vec[i], &node1, &node2);
        RagNode_t* rag_node1 = rag->find_rag_node_no_probe(node1); 
        RagNode_t* rag_node2 = rag->find_rag_node_no_probe(node2); 
        temporary_pointers[i].first = -1.0;
        if (!(rag_node1 && rag_node2)) {
            continue;
        }
        RagEdge_t* rag_edge = rag->find_rag_edge_no_probe(rag_node1, rag_node2);
        ...
        if (valid_edge(rag_edge)) {
            ...
            if (val <= threshold) {
                //ranking_lock.lock();
                //ranking.insert(std::make_pair(val, std::make_pair(node1, node2)));
                temporary_pointers[i] = std::make_pair(val, std::make_pair(node1,node2));
                //ranking_lock.unlock();
            }
            else{ 
            kicked_out++;   
    ...
    for (int i = 0; i < delete_vec.size(); i++) {
      if (temporary_pointers[i].first != -1.0) {
        ranking.insert(temporary_pointers[i]);
      }
    }
    free(temporary_pointers);
}
