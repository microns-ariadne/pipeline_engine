...
void FeatureHist::add_point_batch(std::vector<double>& val, void * cache, unsigned int x , unsigned int y , unsigned int z ) {
        ...
        for (int i = 0; i < val.size(); i++) {
          ++bins[(int)(val[i] * num_bins)];
        }
        for (int i = 0; i < num_bins; i++) {
          hist_cache->hist[i] += bins[i];
        }*/
        hist_cache->count += val.size();
        
        for (int i = 0; i < val.size(); i++) {
          ++(hist_cache->hist[val[i] * num_bins]);
          //++(hist_cache->count);
        }
}
...
void FeatureMoment::add_point_batch(std::vector<double>& val, void * cache, unsigned int x, unsigned int y, unsigned int z){
        MomentCache * moment_cache = (MomentCache*) cache;
        for (int j = 0; j < val.size(); j++) {
          double tmp = val[j];
          for (int i = 0; i < num_moments; ++i) {
              moment_cache->vals[i] += tmp;
              tmp = tmp*val[j];
          }
        } 
        moment_cache->count += val.size();
}
...
void FeatureCount::add_point_batch(std::vector<double>& val, void * cache, unsigned int x, unsigned int y, unsigned int z)
{
