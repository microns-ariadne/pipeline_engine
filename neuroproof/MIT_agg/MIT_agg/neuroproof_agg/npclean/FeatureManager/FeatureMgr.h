...
*/
    void add_val_batch(std::vector<double>& vals, RagNode_t* node, int stride)
    {
        //node->incr_size();
        ...
        if (node_caches.find(node) != node_caches.end()) {
            ...
            for (int j = 0; j < stride; j++) {
              for (int k = 0; k < channels_features[j].size(); k++) {
                //for (int i = 0; i < vals.size(); i++) {
                  //int cache_index = j*channels_features[j].size() + k;
                  channels_features[j][k]->add_point_batch(vals, feature_caches[starting_pos]);
                //}
                starting_pos++;
              }
            }
