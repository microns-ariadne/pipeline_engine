...
class NeuroproofStitchTaskMixin(DatasetMixin):
...
class NeuroproofStitchRunMixin(NeuroproofDilateMixin):
    ...
    dilation_xy = luigi.IntParameter(
        default=7,
        description="The size of the side of the square structuring element "
        "used to dilate the membrane prediction for Neuroproof in the X and Y "
        "directions")
    dilation_z = luigi.IntParameter(
        default=5,
        description="The size of the side of the square structuring element "
        "used to dilate the membrane prediction for Neuroproof in the Z "
        "direction")
    ...
    def ariadne_run(self):
        ...
        try:
            if self.neuroproof_version == NeuroproofVersion.MINIMAL:
                self.run_standard(loading_plan)
            elif self.neuroproof_version == NeuroproofVersion.FLY_EM:
                self.run_optimized_with_copy(loading_plan)
            elif self.neuroproof_version == NeuroproofVersion.FAST:
                self.run_fast(loading_plan)
            else:
                self.run_optimized(loading_plan)
        finally:
            self.delete_dilated_loading_plan(loading_plan)

    def run_standard(self, loading_plan):
        ...
        prob_volume = DestVolumeReader(loading_plan)
    ...
    def run_optimized_with_copy(self, loading_plan):
    ...
    def run_optimized(self, loading_plan):
    ...
    def run_fast(self, loading_plan):
...
class NeuroproofStitchTask(NeuroproofStitchTaskMixin, NeuroproofStitchRunMixin,
