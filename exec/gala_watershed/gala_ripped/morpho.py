...
def remove_small_connected_components(a, min_size=64, in_place=False):
    ...
    print 'remove_small_connected_components: num_of_components = %d' % (len(component_sizes) - sum(too_small),)
...
def refined_seeding(a, maximum_height=0, grey_close_radius=1, 
    ...
    seeds, num_seeds = label(s)
    
    print 'refined_seeding: num_seeds = %d' % (num_seeds,)
